...
userActivity <- synapser::synTableQuery(paste("select * from", masterTable, "WHERE dataGroups NOT LIKE '%test%' AND createdOn > 1502668800000"))
userActivity <- userActivity$asDataFrame()

#Data Manipulation
userActivity <- userActivity %>% 
  dplyr::mutate(createdOnTimeZone = as.integer( (createdOnTimeZone)/100),
                #time is in miliseconds
                createdOn = lubridate::as_datetime(userActivity$createdOn / 1000),
...
p4 <- ggplot(data=pro_surveys_compliance, aes(x=participant_week, y=percent, color=Survey ))  + geom_point() + geom_line(size=1)
...
p5 <- ggplot(data=activeSensor_tasks_compliance, aes(x=participant_week, y=percent, color=sensorActivity ))  + geom_point() + geom_line(size=1)
...
p6 <- ggplot(data=otherShortSurveys_compliance, aes(x=participant_week, y=percent, color=otherSurveys ))  + geom_point() + geom_line(size=1)
