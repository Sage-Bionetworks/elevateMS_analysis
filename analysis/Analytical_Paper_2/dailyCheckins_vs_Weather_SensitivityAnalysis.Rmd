...
prop.table(table(dailyCheckins_flt$pain))
prop.table(table(dailyCheckins_flt$health))
prop.table(table(dailyCheckins_flt$mobility))
...
tmp <- dailyCheckins_N_weather %>% filter(health != 'Horrible') %>% mutate(health = as.character(health))
install_load("lme4", "lmerTest", "broom")
res.1 <- lmer(atmosphericPressure ~ health + (1 | healthCode), data=tmp )
res.2 <- lmer(cloudCoverage ~ health  + (1 | healthCode), data=tmp )
res.3 <- lmer(currentTemperature ~ health + (1 | healthCode), data=tmp )
res.4 <- lmer(humidity ~ health + (1 | healthCode), data=tmp )
res.5 <- lmer(maximumTemperature ~ health + (1 | healthCode), data=tmp )
res.6 <- lmer(minimumTemperature ~ health + (1 | healthCode), data=tmp )
...
#### Mixed effect modelling
install_load("lme4", "lmerTest", "broom")
tmp <- dailyCheckins_N_weather %>% filter(mobility != 'Horrible') %>% mutate(mobility = as.character(mobility))
...
p1 <- ggplot(data=tmp, aes(x=mobility, y=currentTemperature)) + geom_boxplot(width=0.6)
...
#### Mixed effect modelling
install_load("lme4", "lmerTest", "broom")
tmp <- dailyCheckins_N_weather %>% filter(mobility != 'Horrible') %>% mutate(pain = as.character(pain))
res.1 <- lmer(atmosphericPressure ~ pain + (1 | healthCode), data=tmp )
res.2 <- lmer(cloudCoverage ~ pain  + (1 | healthCode),  data=tmp )
res.3 <- lmer(currentTemperature ~ pain + (1 | healthCode), data=tmp )
res.4 <- lmer(humidity ~ pain + (1 | healthCode), data=tmp )
res.5 <- lmer(maximumTemperature ~ pain + (1 | healthCode), data=tmp )
res.6 <- lmer(minimumTemperature ~ pain + (1 | healthCode), data=tmp )
sjPlot::tab_model(res.1, res.2, res.3, res.4, res.5, res.6, show.ci = F)
```
...
### Plot top differentiator - cloudCoverage
p1  <- ggplot(data=tmp, aes(x=pain, y=humidity)) + geom_boxplot(width=0.6)
p1  <- p1 + geom_jitter(size=0.3, width=0.2, color="grey60") + theme_light(base_size = 15)
