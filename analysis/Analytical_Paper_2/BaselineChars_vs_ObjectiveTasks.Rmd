...
datatable(anova.pvals,
          extensions = 'Buttons', 
          options = list(dom = 'Bfrtip', buttons = c('csv', 'excel')),
          rownames = F)


#### get effect size for top feature
# View(association_baselineChars_vs_tapF %>%
#   mutate(glance = lm_res %>% map(broom::tidy)) %>%
#   select(-data, -lm_res, -anova_res) %>%
#   unnest() %>% 
#   filter(feature == bestTapF_PDDS_feature) %>%
#   filter(baselineChar == 'overallPhysicalAbility'))
...
#### get effect size for top feature
# View(association_baselineChars_vs_walkF %>%
#   mutate(glance = lm_res %>% map(broom::tidy)) %>%
#   select(-data, -lm_res, -anova_res) %>%
#   unnest() %>%
#   filter(feature == bestWalkF_PDDS_feature) %>%
#   filter(baselineChar == 'overallPhysicalAbility'))
...
datatable(association_baselineChars_vs_restF %>%
            dplyr::arrange(p.val.adj), 
          extensions = 'Buttons', 
          options = list(dom = 'Bfrtip', buttons = c('csv', 'excel')),
          rownames = F)
...
datatable(anova.pvals %>% dplyr::arrange(p.val.adj), 
          extensions = 'Buttons', 
          options = list(dom = 'Bfrtip', buttons = c('csv', 'excel')),
          rownames = F)

#### get effect size for top feature
# View(association_baselineChars_vs_dsstF %>%
#   mutate(glance = lm_res %>% map(broom::tidy)) %>%
#   select(-data, -lm_res, -anova_res) %>%
#   unnest() %>%
#   filter(feature == bestDSSTF_PDDS_feature) %>%
#   filter(baselineChar == 'overallPhysicalAbility'))
