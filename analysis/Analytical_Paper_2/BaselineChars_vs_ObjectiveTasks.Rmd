...
p <- ggplot(data = df, aes(y=value, x=(baselineValue))) + geom_boxplot(width=0.5) + geom_jitter(size=0.3, width=0.2, color="grey60") + theme_bw(base_size = 15) + xlab('PDDS Category') + ylab("number of taps(median)") + coord_flip()
p
...
df <- baselineChars_vs_tapF %>%
  ...
  filter(!is.na(baselineValue))
p <- ggplot(data = df, aes(y=value, x=(baselineValue))) + geom_boxplot(width=0.5) + geom_jitter(size=0.3, width=0.2, color="grey60") + theme_bw(base_size = 15) +  xlab('Age group') + ylab("number of taps(median)") + coord_flip()
p
...
p <- ggplot(data = df,
       aes(y=log10(value), x=baselineValue)) + geom_boxplot(width=0.5) + geom_jitter(size=0.3, width=0.2, color="grey60") +  theme_bw()  + theme_bw(base_size = 15) + xlab('PDDS Category') + ylab("F0FAJ.median")  + coord_flip()
p
...
bestTremorF_PDDS_feature  <- association_baselineChars_vs_tremorF %>%
  dplyr::filter(baselineChar == 'overallPhysicalAbility') %>%
  dplyr::slice(which.min(p.val.adj)) %>% .$feature %>% as.character()
...
df <- baselineChars_vs_tremorF %>%
  dplyr::filter(baselineChar == "overallPhysicalAbility", feature == 'skewness.tm.IMF1.md_uav_gyroscope.IQR') %>%
  filter(!is.na(baselineValue)) %>%
  dplyr::mutate(baselineValue = gsub('disability', '-disability', baselineValue),
                  baselineValue = factor(baselineValue, 
                                         levels=c('normal', 'mild-disability',
                                                  'moderate-disability', 'gait-disability')))
p <- ggplot(data = df,
       aes(y=value, x=baselineValue)) + geom_boxplot(width=0.5) + geom_jitter(size=0.3, width=0.2, color="grey60") +  theme_bw()  + theme_bw(base_size = 15) + xlab('PDDS Category') + ylab("Tremor - skewness.tm.IMF1.md_uav_gyroscope.IQR")  + coord_flip()
p
ggsave("Figs_N_Tables/BaselinePDDS_vs_TremorF1.jpeg", plot = p, width = 8, height = 6, dpi = 300)
ggsave("Figs_N_Tables/BaselinePDDS_vs_TremorF1.tiff", plot = p, width = 8, height = 6, dpi = 300)
```
  ...
  dplyr::mutate(baselineValue = gsub('disability', '-disability', baselineValue),
                  baselineValue = factor(baselineValue, 
                                         levels=c('normal', 'mild-disability',
                                                  'moderate-disability', 'gait-disability')))
p <- ggplot(data = df,
       aes(y=(value), x=baselineValue)) + geom_boxplot(width=0.5) + geom_jitter(size=0.3, width=0.2, color="grey60") +  theme_bw()  + theme_bw(base_size = 15) + xlab('PDDS Category') + ylab("number correctly called symbols")  + coord_flip()
p
