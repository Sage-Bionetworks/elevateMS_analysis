...
p <- p + xlab('Daily Temperature(F)') + ylab('Number of Taps') + theme_bw(base_size = 15)
p
ggsave("Figs_N_Tables/weatherF_vs_TapF1.jpeg", plot = p, width = 8, height = 6, dpi = 300)
ggsave("Figs_N_Tables/weatherF_vs_TapF1.tiff", plot = p, width = 8, height = 6, dpi = 300)
```
...
p <- ggplot(data=df %>% filter(!is.na(weatherFeatureVal)),
       aes(y=objFeatureVal, x=weatherFeatureVal)) 
p <- p + geom_boxplot() + geom_jitter(size=0.3, width = 0.3, color="gray70") + theme_bw() 
p <- p + xlab('Daily Temperature(F)') + ylab('Number of correctly called digits') + theme_bw(base_size = 15)
p
ggsave("Figs_N_Tables/weatherF_vs_DSST.jpeg", plot = p, width = 8, height = 6, dpi = 300)
ggsave("Figs_N_Tables/weatherF_vs_DSST.tiff", plot = p, width = 8, height = 6, dpi = 300)
```
...
features_that_failed_modelling
...
df <- tremorF_and_weatherF %>% 
  dplyr::filter(objFeature == "energy.tm.IMF1.iqr_uj_accelerometer" , weatherFeature == 'currentTemperature') %>%
  mutate(weatherFeatureVal = cut(weatherFeatureVal, breaks = c(0,50, 80, 130), 
                                 labels = c('0-50', '51-80', '80+')))
p <- ggplot(data=df %>% filter(!is.na(weatherFeatureVal)),
       aes(y=log10(objFeatureVal), x=weatherFeatureVal)) 
p <- p + geom_boxplot() + geom_jitter(size=0.3, width = 0.3, color="gray70") + theme_bw() 
p <- p + xlab('Daily Temperature(F)') + ylab('log10(energy.tm.IMF1.iqr_uj_accelerometer)') + theme_bw(base_size = 15)
p
ggsave("Figs_N_Tables/weatherF_vs_TremorF1.jpeg", plot = p, width = 8, height = 6, dpi = 300)
ggsave("Figs_N_Tables/weatherF_vs_TremorF1.tiff", plot = p, width = 8, height = 6, dpi = 300)
```
