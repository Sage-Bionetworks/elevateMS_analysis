...
* Group's proportion: `r prop.table(table(baselineChar$dataGroups)) * 100`
* Numbers in three groups: `r table(baselineChar$group)`
...
demogTable = tableone::CreateTableOne(data=baselineChar_flt, vars=listVars, factorVars = catVars, strata = c("group"))
demogTableMat <- print(demogTable, quote = FALSE, noSpaces = TRUE, printToggle = FALSE)
## Save to a CSV file
write.csv(demogTableMat, file = "elevateMS_overall_demog_table.csv")

table1(~ gender + race + education + health_insurance + employment | group, 
       data=baselineChar_flt,overall="Total")
```
...
### Demographic Summary (without missing data)

#Create a variable list which we want in Table 1
listVars <- c("age", "gender", "race", "education", "health_insurance", 
              "employment")
baselineChar_flt_noNA <- baselineChar_flt %>% 
  tidyr::gather(demog, demogVal, listVars) %>%
  dplyr::filter(!( is.na(demogVal) | demogVal == '' | demogVal == 'NULL' | demogVal == '<NA>')) %>%
  tidyr::spread(demog, demogVal)


View(baselineChar_flt_noNA)


table1(~ gender + race + education + health_insurance + employment | group, 
       data=baselineChar_flt_noNA,overall="Total")
```
...
demogTable = CreateTableOne(data=tmp_data, 
