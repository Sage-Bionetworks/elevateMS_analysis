...
#Number of tapping activities 
# nTapAct <- tmp_tapF %>% group_by(healthCode, hand, feature) %>% 
#   dplyr::summarise(n = n()) %>%
#   select(-feature) %>%
#   distinct()
...
cor_tapF_nQOL_cog <- tmp_calc_cor(tapF_week_averaged, nQOL_cognition_week_avg, use_first_common_week = F, 
                                  additional_group_by = 'hand') %>% mutate(survey = 'neuroQOL Cognition')

cor_tapF_nQOL_uppExt <- tmp_calc_cor(tapF_week_averaged, nQOL_uppExtremity_week_avg, use_first_common_week = F, 
                                  additional_group_by = 'hand') %>% mutate(survey = 'neuroQOL Upper Extremity')

cor_tapF_nQOL_lowExt <- tmp_calc_cor(tapF_week_averaged, nQOL_lowExtremity_week_avg, use_first_common_week = F, 
                                     additional_group_by = 'hand') %>% mutate(survey = 'neuroQOL Lower Extremity')

cor_tapF_allWeeks <- rbind(cor_tapF_nQOL_cog, cor_tapF_nQOL_lowExt, cor_tapF_nQOL_uppExt)

library(grid)
p1 <- ggplot(cor_tapF_allWeeks, aes(x=cor, y=-log10(p.val.adj), color=hand)) + theme_few() + geom_point(size=.7)  + facet_grid( . ~ survey )
...
## Cor taps - First common week where tapping and neuroQol's was completed
cor_tapF_nQOL_cog_firstCommonWeek <- tmp_calc_cor(tapF_week_averaged, nQOL_cognition_week_avg, use_first_common_week = T, 
                                  additional_group_by = 'hand') %>% mutate(survey = 'neuroQOL Cognition')

cor_tapF_nQOL_uppExt_firstCommonWeek <- tmp_calc_cor(tapF_week_averaged, nQOL_uppExtremity_week_avg, use_first_common_week = T, 
                                     additional_group_by = 'hand') %>% mutate(survey = 'neuroQOL Upper Extremity')

cor_tapF_nQOL_lowExt_firstCommonWeek <- tmp_calc_cor(tapF_week_averaged, nQOL_lowExtremity_week_avg, use_first_common_week = T, 
                                     additional_group_by = 'hand') %>% mutate(survey = 'neuroQOL Lower Extremity')

cor_tapF_firstCommonWeek <- rbind(cor_tapF_nQOL_cog_firstCommonWeek, cor_tapF_nQOL_lowExt_firstCommonWeek, cor_tapF_nQOL_uppExt_firstCommonWeek)

p2 <- ggplot(cor_tapF_firstCommonWeek, aes(x=cor, y=-log10(p.val.adj), color=hand)) + theme_few() + geom_point(size=.7)  + facet_grid( . ~ survey )
...
ggsave("analysis/Analytical_Paper_2/FIGURES/tapping_n_NeuroQOLs_top_2_features_scatterPlot_allWeeks.png", p3, height=3, width=8, units="in", dpi=300)



p1 <- get_scatter_plot(tapF_week_averaged %>% filter(hand == 'left'), nQOL_uppExtremity_week_avg, 'numberTaps', use_first_common_week = T)
p2 <- get_scatter_plot(tapF_week_averaged %>% filter(hand == 'left'), nQOL_uppExtremity_week_avg, 'meanTapInter', use_first_common_week = T)
p3 <- grid.arrange(p1,p2, nrow=1)
ggsave("analysis/Analytical_Paper_2/FIGURES/tapping_n_NeuroQOLs_top_2_features_scatterPlot_firstCommonWeek.png", p3, height=3, width=8, units="in", dpi=300)
