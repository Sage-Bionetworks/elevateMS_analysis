...
# survey <- nQOL_uppExtremity_week_avg %>% select(healthCode, TScore, participant_week)
# features <- tapF_week_averaged %>% spread(feature, value)
# tmp_df <- merge(features, survey)
# 
# x <- tmp_df %>% select(healthCode, meanTapInter, TScore ) %>%
#   mutate(meanTapInterRank = rank(meanTapInter),
#          TScoreRank= rank(TScore))  %>%
#   filter(TScore <= 50)
# 
# hist(tmp_df$TScore)
# cor.test(x$TScore, x$meanTapInter, method="spearman")
# 
# 
# 
# ggplot(data=x, aes(x=meanTapInterRank, y=TScore)) + geom_hex()
