...
userActivity <- userActivity %>% 
  dplyr::filter(healthCode %in% SELECTED_USERS,
                dataGroups %in% c('ms_patient', 'control'))
...
#Retention by clinical referrals - NOT NEEDED AS TIME TO EVENT ANALYSIS(SURVIVAL ANALYSIS) DOES IT.
...
#### New retention plot
# NOT NEEDED AS TIME TO EVENT ANALYSIS(SURVIVAL ANALYSIS) DOES IT.
...
p4 <- p4 +  ggtitle('Weekly surveys')  + xlab('#weeks enrolled') +  theme(text = element_text(size=15))
...
p5 <- p5 +  ggtitle('Sensor tasks')  + xlab('#weeks enrolled') +  theme(text = element_text(size=15))
p5 <- p5 + scale_x_discrete(limits=seq(1,12,1)) + scale_y_continuous(limits=c(1,100))
p5
ggsave(file="analysis/Analytical_Paper_2/Figs_N_Tables/Supp_activeTasks_retention.png", plot=p5,
...
p6
ggsave(file="analysis/Analytical_Paper_2/Figs_N_Tables/otherShort_surveys_retention.png", plot=p6,
       width=8, height = 5, units="in", dpi=300)
...
p7
ggsave(file="analysis/Analytical_Paper_2/Figs_N_Tables/perWeekActivity.jpeg", plot=p7,
...
####################
# The Following code needs to be cleaned and re-run if/when needed
####################
...
# baselineChar <- baselineChar %>% as.data.frame()
