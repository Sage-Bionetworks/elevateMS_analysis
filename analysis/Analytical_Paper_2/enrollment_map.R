...
baselineChar <- as.data.frame(baselineChar)

#create the new col to match to tmp_zipcode
baselineChar['zipcode_firstThree'] = substr(baselineChar$zipcode, 1,3)
baselineChar$zipcode_firstThree
tmp_zipcode$zipcode_firstThree
tmp_zipcode$state <- NULL
tmp_user_region <- merge(baselineChar, tmp_zipcode) %>% filter(dataGroups %in% c('control', 'ms_patient'))
...
p <- plot_geo(tmp_user_region, locationmode = 'USA-states') %>%
  add_markers( x = ~longitude, y = ~latitude, color=~dataGroups, opacity=.7, 
               colors=c(COL_CONTROL, COL_MS_PATIENT),
               marker=list(sizeref=1)) %>%
  layout(title='', geo=g) %>% layout(legend = list(x = 0.9, y = 0.9))
p
if (!require("processx")) install.packages("processx")
orca(p, "analysis/Analytical_Paper_2/Figs_N_Tables/enrollmentMap.svg")
