...
dsstF <- dsst %>% inner_join(baselineChar %>% select(healthCode, dataGroups)) %>%
  filter(dataGroups == 'ms_patient' & 
         percentCorrect >= 80) %>%
  gather(feature, value, c(3,5,6,7,8,9)) 


dsstF_week_averaged =  dsstF %>% select(healthCode, participant_week, feature, value) %>%
...
p1
ggsave("analysis/Analytical_Paper_2/FIGURES/dsstF_n_NeuroQOLs_corr_allWeeks_volcanoPlot.jpeg", plot=p1, height=4, width=10, units="in", dpi=300)
...
ggsave("analysis/Analytical_Paper_2/FIGURES/dsstF_top_2_feature_scatterPlot_allWeek.png", p3, height=3, width=9, units="in", dpi=150)
...
View(neurCog_firstCommonWeek_dsstF)
p1 <- ggplot(dsstF_week_averaged, aes(x=cor, y=-log10(p.val.adj))) + theme_few() + geom_point(size=.7)  + facet_grid( . ~ survey )
p1
...
p <- p + theme_few()
p
ggsave("analysis/Analytical_Paper_2/FIGURES/dsst_feature_numCorrect_physicalDisability_boxplot.png", p, height=4, width=8, units="in", dpi=300)
