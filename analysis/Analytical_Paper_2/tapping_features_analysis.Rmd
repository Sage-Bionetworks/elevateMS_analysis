...
```{r}
##TAP FEATURE COLS -
#cat(colnames(tapF), sep="', '")
TAP_FEATURES <- c('meanTapInter', 'medianTapInter', 'iqrTapInter', 'minTapInter', 'maxTapInter', 'skewTapInter', 'kurTapInter', 'sdTapInter', 'madTapInter', 'cvTapInter', 'rangeTapInter', 'tkeoTapInter', 'ar1TapInter', 'ar2TapInter', 'fatigue10TapInter', 'fatigue25TapInter', 'fatigue50TapInter', 'meanDriftLeft', 'medianDriftLeft', 'iqrDriftLeft', 'minDriftLeft', 'maxDriftLeft', 'skewDriftLeft', 'kurDriftLeft', 'sdDriftLeft', 'madDriftLeft', 'cvDriftLeft', 'rangeDriftLeft', 'meanDriftRight', 'medianDriftRight', 'iqrDriftRight', 'minDriftRight', 'maxDriftRight', 'skewDriftRight', 'kurDriftRight', 'sdDriftRight', 'madDriftRight', 'cvDriftRight', 'rangeDriftRight', 'numberTaps', 'buttonNoneFreq', 'corXY')
...
tmp_plot_permute_results <- function(permuteResuls, select_neruoQoL_survey){
  ...
  p <- p + theme_bw() + xlab('distribution of 10,00 random permutations') + ggtitle(select_neruoQoL_survey)
...
weatherF_flt <- weatherF %>%
  ...
  dplyr::select(healthCode, weather_date, atmosphericPressure, cloudCoverage, 
                currentTemperature, humidity, maximumTemperature, minimumTemperature) %>%
  dplyr::group_by(healthCode, weather_date) %>%
  dplyr::summarise_all(.funs = function(x) mean(x, na.rm = T))

tapF_flt <- tapF %>%
  dplyr::mutate(tap_date = as.Date(activity_start_timestamp_local)) 

tap_N_weather <- merge(tapF_flt, weatherF_flt, 
                       by.x=c('healthCode', 'tap_date'), 
                       by.y=c('healthCode', 'weather_date')) %>%
  distinct() 

tap_N_weather_long <- tap_N_weather %>%
  tidyr::gather(tapF, tapF_val, c(TAP_FEATURES, activityDuration_seconds)) %>%
  tidyr::gather(weatherF, weatherF_val, 
                c("atmosphericPressure", "cloudCoverage", "currentTemperature", "humidity", 
                  "maximumTemperature", "minimumTemperature"))

tmp_cor <- function(x,y){
  cor.test(x, y, use="complete.obs", method="spearman")
}

#select healthCodes with 10 tap activities where weather data is available
selectHealthCodes <- tap_N_weather %>% count(healthCode, hand) %>% filter(n >= 10) %>% .$healthCode %>% unique()

tap_N_weather_cor <- tap_N_weather_long %>% 
  filter(tapF %in% c('medianTapInter', 'numberTaps')) %>%
  filter(healthCode %in% selectHealthCodes) %>%
  dplyr::group_by(healthCode, hand,  tapF, weatherF) %>%
  do(corTest = tmp_cor(.$weatherF_val, .$tapF_val))
