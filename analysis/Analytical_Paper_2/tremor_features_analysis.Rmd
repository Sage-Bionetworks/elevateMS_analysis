...
### Impact of Weather on Tremor test performance (early analysis)
...
tremorF_flt <- tremorF %>%
  dplyr::mutate(tremorTest_date = as.Date(activity_start_timestamp_local)) 

tremor_N_weather <- merge(tremorF_flt, weatherF_flt, 
                       by.x=c('healthCode', 'tremorTest_date'), 
                       by.y=c('healthCode', 'weather_date')) %>%
  distinct() 

tremor_N_weather_long <- tremor_N_weather %>%
  tidyr::gather(tremorF, tremorF_val, c(TREMOR_FEATURES)) %>%
...
View(tap_N_weather_cor %>% broom::tidy(corTest) )
