...
```{r}
# tmp_calc_cor <- function(features, survey, additional_group_by=F){
#   
...
tmp_calc_assoc <- function(features, survey, hand=F){
  ...
  if(hand == T){
    ...
    group_by(feature) %>%
    nest() %>%
    dplyr::mutate(mod = map(data, function(df) lmer(value ~ class  + hand + (1 | healthCode), data=df)))
  } else {
    res <- tmp_df %>%
    group_by(feature) %>%
    nest() %>%
    dplyr::mutate(mod = map(data, function(df) lmer(value ~ class + (1 | healthCode), data=df)))
  } 
...
top_features <- function(df){
  
  df %>%
    filter(lmm.anova.pval.adj <= .05) %>%
    select(survey, feature, lmm.mod.res, lmm.anova.pval, lmm.anova.pval.adj) %>%
    unnest() %>%
    filter(str_detect(term, 'class')) %>%
    mutate(lmm.pval = round(p.value, digits=5), 
    ...
    select(-group, -statistic, -df, -p.value)
}
...
#features associated with FDR < .05
top_tapF_associated_with_nQOLs <- top_features(tapF_association)

datatable(top_tapF_associated_with_nQOLs, 
          extensions = 'Buttons', 
...
top_walkF_associated_with_nQOLs <- top_features(walkF_association)

datatable(top_walkF_associated_with_nQOLs, 
          extensions = 'Buttons', 
          options = list(dom = 'Bfrtip', buttons = c('csv', 'excel')),
          rownames = F)
```
...
top_restF_associated_with_nQOLs <- top_features(restF_association)

datatable(top_restF_associated_with_nQOLs, 
          extensions = 'Buttons', 
          options = list(dom = 'Bfrtip', buttons = c('csv', 'excel')),
          rownames = F)
```
...
df <- df$data[[1]]
ggplot(data=df, aes(x=class, y=log10(value))) + geom_boxplot(width=0.5) + geom_jitter(size=0.3, width=0.2, color="grey60") + theme_bw() + xlab('NeuroQoL Cognition') + ylab('lgo10(P0FY)') + theme_bw(base_size = 15)
...
top_dsstF_associated_with_nQOLs <- top_features(dsstF_association)

datatable(top_dsstF_associated_with_nQOLs, 
          extensions = 'Buttons', 
          options = list(dom = 'Bfrtip', buttons = c('csv', 'excel')),
          rownames = F)
```
...
df <- df$data[[1]]
p <- ggplot(data=df, aes(x=class, y=log10(value))) + geom_boxplot(width=0.5) + geom_jitter(size=0.3, width=0.2, color="grey60") + theme_bw() + xlab('NeuroQoL Cognition') + ylab('lgo10(Average time)') + theme_bw(base_size = 15)
c
