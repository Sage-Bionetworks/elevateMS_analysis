############################################################################
# ElevateMS project
# Purpose: Find Age Matched health codes(elevateMS + mpower controls)[without filtering for gender]
...
hc.nc.ms <- tremor.tbl.ms %>% 
  ...
  dplyr::left_join(demo.tbl.ms %>% # Also rename dataGroups to MS
                     dplyr::select(healthCode, MS = dataGroups, age)) %>% 
  dplyr::filter(!is.na(MS), !is.na(age)) %>% # remove NAs
  dplyr::filter(MS %in% c('ms_patient','control')) %>% 
  unique() %>% 
  droplevels() %>% 
  dplyr::count(healthCode, age, MS) %>% 
  unique()

# High frequency filter (atleast 2 records)
hc.hfreq.ms = hc.nc.ms %>% na.omit() %>% 
  dplyr::select(healthCode, age, MS, n) %>% 
...
hc.nc.mpower <- tremor.tbl.mpower %>% 
  ...
  dplyr::left_join(demo.tbl.mpower %>% # Also rename inferred_diagnosis to MS
                     dplyr::select(healthCode, MS = inferred_diagnosis, age)) %>% 
  dplyr::filter(!is.na(MS), !is.na(age)) %>% # remove NAs
...
age.records.matched = plyr::ddply(dat, .(subclass), .fun = function(x){
  dat1 = x %>%
    dplyr::select(healthCode, MS, n, age) %>%
    unique() 
...
thisFileName <- "analysis/Tremor_analysis/no_gender/age_match_healthCodes_no_gender.R" # location of file inside github repo
...
OUTPUT_FILE <- "age_match_healthCodes_no_gender.tsv" # name your file
