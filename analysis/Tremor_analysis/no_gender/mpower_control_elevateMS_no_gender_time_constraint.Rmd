...
knit2synapse::createAndKnitToFolderEntity(file = "./mpower_control_elevateMS_no_gender.Rmd",
                                 ...
                                 folderName = "elevateMS action tremor test (mpower PD action tremor train)(no scaling)(atleast 2 records)(no gender)(time constraint)")
```
...
thisFileName <- 'mpower_control_elevateMS_no_gender_time_constraint.Rmd'
...
rfModelCaseControl <- function(dat, responseNames, contFeatures, alwaysSplitVariables){
  ...
  tryCatch({
    ...
    pred.probabilities = rowSums(rf.prediction$predictions == dat.test$MS, na.rm = T)/rf.prediction$num.trees
...
summarized.ftrs.elevateMS.record = read.csv(synapser::synGet(summarized.ftrs.elevateMS.id.record)$path,
                                     sep = '\t')
all.used.ids = c(all.used.ids, summarized.ftrs.elevateMS.id, summarized.ftrs.elevateMS.id.record)
###########################################################
...
dat.test <- summarized.ftrs.elevateMS.record %>%
  dplyr::filter((healthCode %in% age.records.matched$healthCode)) %>%
    dplyr::select(healthCode,recordId,one_of(kineticFeaturesToSelect)) %>% 
  dplyr::left_join(demo.tbl.ms %>%
                     dplyr::select(healthCode, gender, age, MS = dataGroups)) %>%
  dplyr::filter(MS %in% c('ms_patient','control'),
                gender %in% c('male','female'),
...
CODE = Folder(name = 'elevateMS action tremor test (mpower PD action tremor train)(no scaling)(atleast 2 records)(no gender)(time constraint)', 
              parentId = "syn19120636")
CODE = synStore(CODE)
