...
# more MS data from baseline characteristics
baseline.char.MS <- read.csv(synapser::synGet('syn17115631')$path, sep = '\t') %>% 
  dplyr::select(healthCode, age, gender) %>% 
  na.omit() %>% 
  unique()

tap.demo.MS <- tap.demo.MS %>% 
  dplyr::full_join(baseline.char.MS) %>% 
  unique()
