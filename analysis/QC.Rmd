...
pro_surveys_compliance['Neuro_QOL_type'] = pro_surveys_compliance$originalTable
pro_surveys_compliance$originalTable <- NULL
pro_surveys_compliance['Neuro_QOL_type'] = gsub('NeuroQOL-', '',pro_surveys_compliance$`Neuro_QOL_type`)
p4 <- ggplot(data=pro_surveys_compliance, aes(x=participant_week, y=percent, color=Neuro_QOL_type ))  + geom_point() + geom_line(size=1.5)
...
otherShortSurveys <-  userActivity %>% 
  filter( (originalTable %like% 'Daily Check' | originalTable %like% 'Relapses Survey' | originalTable %like%  'Symptoms Survey' ) & dataGroups == 'ms_patient' )
...
p6 <- p6 + theme_few() +  scale_color_manual(values=c( "#2EC4B6", "#E71D36", "#FF9F1C")) 
...
#for each user find the last entered date of demog info
selected_vals <- demog %>% mutate(startDate = lubridate::ymd_hms(startDate)) %>%
  dplyr::group_by(healthCode) %>% 
  dplyr::summarise(startDate = max(startDate))
demog <- merge(demog, selected_vals)
...
* Demographics data avaialble for : `r  n_distinct(demog$healthCode)` individuals | `r percent_demog_data_compliance` %


```{r}
gender = demog %>% inner_join(tmp_df) %>% dplyr::group_by(dataGroups, gender, num) %>% dplyr::summarise(prop =n_distinct(healthCode)) %>% mutate(prop = round((prop / num)*100, digits=2))
...
```{r}
employment = demog %>% inner_join(tmp_df) %>% dplyr::group_by(dataGroups, employment, num) %>% dplyr::summarise(prop =n_distinct(healthCode)) %>% mutate(prop = round((prop / num)*100, digits=2))
...
p1 <- p1 + theme(text = element_text(size=15)) + ggtitle('Employment type')
```
...
tmp_user_region <- merge(demog, zipcode)
...
p <- p + geom_point( data=tmp_user_region, aes(x=longitude, y=latitude, color=dataGroups), size=1.6) + theme_few()
