...
userCompliance_data <- userActivity %>% 
  ...
  mutate(activity = case_when(
    ...
    grepl('Triggers', originalTable) ~ 'MS triggers',
    grepl('Symptoms', originalTable) ~ 'MS symptoms',
    grepl('Tapping', originalTable) ~ 'tapping',
    grepl('Brain Baseline-v1', originalTable) ~ 'cognition',
    grepl('NeuroQOL|Medication|MSIS|Profile|WPAI', originalTable) ~ 'Other surveys'
  ))
...
perUser_activitySpecificCompliance_perweek <- userCompliance_data %>%
  mutate(activitytype = case_when(
    ...
    activity %in% c('MS symptoms', 'MS triggers') ~ 'triggers_symptoms'
  )) %>%
  filter(!is.na(activitytype)) %>%
  mutate(activitytype = factor(activitytype, levels = c('daily_checkins',
                                                        'triggers_symptoms',
                                                        'sensor_based'))) %>%
  filter(participant_week <= 12 & dataGroups == 'ms_patient') %>%
  count(healthCode, participant_week, activitytype) %>% 
  mutate(participant_week = as.factor(participant_week))

#This is only calculating for weeks where there is data
...
perUser_totalN_acitivites_completed <- perUser_activitySpecificCompliance_perweek %>%
  group_by(healthCode) %>%
  summarise(total= sum(n)) 

perUser_perAcitivity_total <- perUser_activitySpecificCompliance_perweek %>% 
  inner_join(perUser_totalN_acitivites_completed) %>%
...
p
ggsave("analysis/engagement_Paper_1/FINAL_FIGS/perUser_activityPercent_acrossStudy.png", plot=print(p), height = 5, width = 6, units="in", dpi=100)
ggsave("analysis/engagement_Paper_1/FINAL_FIGS/perUser_activityPercent_acrossStudy.tiff", plot=print(p), height = 5, width = 6, units="in", dpi=200)
...
users_LastWeek_inStudy <- userCompliance_data %>% group_by(healthCode, dataGroups, referred_by_clinician ) %>%
...
perWeek_activeUsers_inStudy <- userCompliance_data %>%
  mutate(activitytype = case_when(
    activity %in% c('finger to nose', 'walking', 'tapping', 'cognition') ~ 'sensor_based',
    activity %in% c('MS symptoms', 'MS triggers', 'Daily checkin', 'Other surveys') ~ 'survey_based'
  )) %>%
  filter(!is.na(activitytype)) %>% 
...
compliance <- merge(totalUsers_byWeek, perWeek_activeUsers_inStudy) %>%
...
p1 <- p1 + scale_color_manual(values=c(COL_MS_PATIENT, COL_MS_PATIENT_CLINICAL_REF, COL_CONTROL))
