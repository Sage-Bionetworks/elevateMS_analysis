...
featureWise_survival <- userRetention %>% filter(dataGroups == 'ms_patient') %>%
  ...
  tidyr::gather(feature, value, 3:11) %>%
  nest(-feature) %>%
  dplyr::mutate(coxPH_model = map(data, run_coxPH_model, c('group', 'value')),
                coxPH_model_test = map(coxPH_model,cox.zph),
                anova_coxPH_model = map(coxPH_model, anova)) 

### NOTE - if you get this error run again with a clean environment
# Error in mutate_impl(.data, dots) : 
#   Evaluation error: argument is not interpretable as logical.
